...
function printReport(results, context) {
  ...
  console.log('Aliyun DirectMail dry-run verification')
...
async function main() {
  ...
  results.push(
    check(
      'Aliyun DirectMail dry run is enabled',
      provider.aliyunDirectMail?.dryRun === true,
      `dryRun=${provider.aliyunDirectMail?.dryRun}`,
    ),
  )
  ...
  const session = await request('/api/auth/login', {
    method: 'POST',
    body: {
      email: `aliyun-dryrun-owner-${runId}@example.com`,
      displayName: 'Aliyun Dryrun Owner',
    },
  })
  ...
  context.inviteEmail = `aliyun-dryrun-editor-${runId}@example.com`
  ...
  results.push(
    check(
      ...
      String(delivery?.providerMessageId || '').startsWith('dryrun_aliyun_'),
...
main().catch((error) => {
  console.error('Aliyun DirectMail dry-run verification failed')
